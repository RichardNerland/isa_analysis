...
preset_scenarios = {
    ...
    'TVET Baseline': {
        ...
        'degrees': {'BA': 0.0, 'MA': 0.0, 'VOC': 0.45, 'NURSE': 0.45, 'NA': 0.1, 'LABOR': 0.0}
    },
    'TVET Conservative': {
        'description': 'Conservative TVET scenario with more vocational degrees.',
        'degrees': {'BA': 0.0, 'MA': 0.0, 'VOC': 0.60, 'NURSE': 0.20, 'NA': 0.20, 'LABOR': 0.0}
    },
    'Labor Baseline': {
    ...
    dcc.Tabs([
        dcc.Tab(label='About', children=[
            html.Div([
                ...
                html.P([
                    "We account for the possibility that some graduates return to their home country after completing (or dropping out of) their program:"
                ]),
                html.Ul([
                    html.Li("The home_prob parameter determines the likelihood of returning."),
                    html.Li("Once home, a student's earnings are much lower, reflecting typical wages in developing economies."),
                    html.Li("The No Advancement (NA) category defaults to a 100% home return to signify no sustained earnings improvement or local job market constraints in the home country.")
                ]),
                html.P([
                                            ...
                                            html.Div([
                                                ...
                                                html.Div([
                                                    ...
                                                    dcc.RangeSlider(
                                                        ...
                                                        step=5,
                                                        value=[0, 30],
                                                        marks={i: f'{i}%' for i in range(0, 41, 10)},
                                                    )
                                                ], style={'marginBottom': '20px'}),
...
def update_from_preset(preset_name, program_type):
    ...
    if triggered_id == 'program-type':
        if program_type == 'University' and preset_name in ['TVET Baseline', 'TVET Conservative', 'Labor Baseline', 'Labor Conservative']:
            preset_name = 'Baseline'
        elif program_type == 'TVET' and preset_name in ['Baseline', 'Conservative', 'Optimistic', 'Labor Baseline', 'Labor Conservative']:
            preset_name = 'TVET Baseline'
        elif program_type == 'Labor' and preset_name in ['Baseline', 'Conservative', 'Optimistic', 'TVET Baseline', 'TVET Conservative']:
            preset_name = 'Labor Baseline'
    ...
    content_elements = [
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.P(f"Annual Employment Rate: {results['employment_rate']*100:.1f}%"),
                html.P(f"Ever Employed Rate: {results.get('ever_employed_rate', 0)*100:.1f}%"),
...
def run_monte_carlo_simulation(n_clicks, num_sims, complexity, 
    ...
    for params in param_grid:
        ...
        sim_result = run_simple_simulation(
            ...
            initial_inflation_rate=inflation_rate / 100.0,
            home_prob=params['leave_labor_force'] / 100.0
        )
...
def run_blended_monte_carlo(n_clicks, num_sims, 
    ...
    for i in range(num_sims):
        ...
        sim_params['home_prob'] = np.random.uniform(min_leave_labor_force, max_leave_labor_force) / 100.0
        ...
        try:
            ...
            results.append({
                ...
                'leave_labor_force': sim_params.get('home_prob', 0) * 100,
    ...
    viz_elements.append(dcc.Graph(figure=irr_fig))
    
    # 2. Summary Statistics Table
    stats_data = [
        {"Statistic": "Mean IRR", "Value": f"{results_df['investor_irr'].mean():.2f}%"},
        {"Statistic": "Median IRR", "Value": f"{results_df['investor_irr'].median():.2f}%"},
        {"Statistic": "Standard Deviation", "Value": f"{results_df['investor_irr'].std():.2f}%"},
        {"Statistic": "10th Percentile", "Value": f"{results_df['investor_irr'].quantile(0.1):.2f}%"},
        {"Statistic": "25th Percentile", "Value": f"{results_df['investor_irr'].quantile(0.25):.2f}%"},
        {"Statistic": "75th Percentile", "Value": f"{results_df['investor_irr'].quantile(0.75):.2f}%"},
        {"Statistic": "90th Percentile", "Value": f"{results_df['investor_irr'].quantile(0.9):.2f}%"},
        {"Statistic": "Minimum IRR", "Value": f"{results_df['investor_irr'].min():.2f}%"},
        {"Statistic": "Maximum IRR", "Value": f"{results_df['investor_irr'].max():.2f}%"},
        {"Statistic": "Probability of IRR > 5%", "Value": f"{(results_df['investor_irr'] > 5).mean()*100:.1f}%"},
        {"Statistic": "Probability of IRR > 8%", "Value": f"{(results_df['investor_irr'] > 8).mean()*100:.1f}%"},
        {"Statistic": "Probability of IRR < 0%", "Value": f"{(results_df['investor_irr'] < 0).mean()*100:.1f}%"}
    ]
    ...
    viz_elements.append(html.Div([
        html.H5("IRR Distribution Statistics", style={'marginTop': '30px', 'marginBottom': '15px'}),
        stats_table
    ]))
    
    # 3. Scenario Distribution Pie Chart
    ...
    scenario_pie = go.Figure(data=[go.Pie(
        labels=[scenario_labels.get(s, s) for s in scenario_counts.index],
        values=scenario_counts.values,
        hole=.3,
        marker_colors=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']
    )])
    
    scenario_pie.update_layout(
        title='Distribution of Selected Scenarios',
        template='plotly_white'
    )
    
    viz_elements.append(dcc.Graph(figure=scenario_pie))
    
    # 4. IRR by Scenario Box Plot
    ...
    viz_elements.append(dcc.Graph(figure=scenario_box))
    
    # 5. Scatter Plot of Leave Labor Force vs IRR colored by Scenario
    llf_scatter_fig = go.Figure()
    ...
    viz_elements.append(dcc.Graph(figure=llf_scatter_fig))
    
    # 6. Scatter Plot of Wage Penalty vs IRR colored by Scenario
    wp_scatter_fig = go.Figure()
    ...
    viz_elements.append(dcc.Graph(figure=wp_scatter_fig))
    
    # 7. Simulation Parameters Summary
    param_data = [
        {"Parameter": "Number of Simulations", "Value": f"{len(results_df)}"},
        {"Parameter": "Program Type", "Value": f"{program_type}"},
        {"Parameter": "Scenario 1", "Value": f"{scenario_labels.get(scenario1_type, scenario1_type)} ({scenario1_weight}%)"},
        {"Parameter": "Scenario 2", "Value": f"{scenario_labels.get(scenario2_type, scenario2_type)} ({scenario2_weight}%)"},
        {"Parameter": "Scenario 3", "Value": f"{scenario_labels.get(scenario3_type, scenario3_type)} ({scenario3_weight}%)"},
        {"Parameter": "Leave Labor Force Range", "Value": f"{min_leave_labor_force}% to {max_leave_labor_force}%"},
        {"Parameter": "Wage Penalty Range", "Value": f"{min_wage_penalty}% to {max_wage_penalty}%"},
        {"Parameter": "Wage Penalty Adjustment", "Value": "+20% (applied to all wage penalties)"}
    ]
    
    param_table = dash_table.DataTable(
        data=param_data,
        columns=[{"name": col, "id": col} for col in param_data[0].keys()],
        style_cell={'textAlign': 'left', 'padding': '10px'},
        style_header={
            'backgroundColor': 'rgb(230, 230, 230)',
            'fontWeight': 'bold'
    ...
    viz_elements.append(html.Div([
        html.H5("Simulation Parameters", style={'marginTop': '30px', 'marginBottom': '15px'}),
        param_table
    ]))
